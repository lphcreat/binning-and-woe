...
def woe_transform(df,label):
    ...
    save_path = _data_dir / 'woe_iv.xlsx'
...
def _list_to_interval():
    f = open(_data_dir / 'split_code_fix.json', 'r')
    self_thres=json.load(f)
    new_intervals=dict()
    for item in self_thres:
        interval=item['split_box']
        try:
            interval=sorted(interval)
            intervals=[[interval[i], interval[i+1]] for i in range(len(interval)-1)]
        except:
            intervals=[0,0]
        new_intervals[item['col_name']]=intervals
    with open(_data_dir / 'split_num.json', "w") as fw:
        json.dump(new_intervals, fw)
